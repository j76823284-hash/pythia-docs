...
|---|---|---|
| `oovp.oracle` | Optimistic oracle — OOv2 (request/propose) + OOv3 (assert) flows. | [oovp.oracle](oovp-oracle.md) |
| `oovp.voting` | DVM — commit-reveal, stake-weighted dispute resolution. | [oovp.voting](oovp-voting.md) |
| `oovp.stake` | Staking, delegation, emissions, slashing, vote-locking. | [oovp.stake](oovp-stake.md) |
| `oovp.govern` | Governance proposals executed via oracle assertions. | [oovp.govern](oovp-govern.md) |
| `oovp.store` | Fee collection and per-collateral final-fee schedule. | [oovp.store](oovp-store.md) |
| `oovp.finder` | Service locator + identifier/collateral/contract registries. | [oovp.finder](oovp-finder.md) |
| `oovp.token` | OOVP protocol token (eosio.token + ANT-style burn/close). | [oovp.token](oovp-token.md) |
| `oovp.reward` | Generic pre-funded reward vault for app integrations. | [oovp.reward](oovp-reward.md) |

## Dependency / deploy order

```
oovp.token → oovp.finder → oovp.store → oovp.stake → oovp.voting → oovp.oracle → oovp.govern → oovp.reward
```
...
* **Fixed point.** Prices and amounts are integers in the token/identifier precision. OOVP is 4 decimals (`1.0000 = 10000`).
