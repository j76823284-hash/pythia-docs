...
```
   ...
   Requester  ───────────────────►  Answer  ──────────────────────────►  Settled
                                       ...
                                       ▼
                                 DVM (oovp.voting)  ──►  stakers vote  ──►  Settled
```

## The two layers

### 1. The optimistic layer — `oovp.oracle`
...
* **currency** — the token used for the bond. Must be whitelisted in `oovp.finder`.
...
### 2. The dispute layer — the DVM (`oovp.voting`)

The DVM is a token-weighted **commit-reveal** voting system that resolves disputes escalated from the optimistic layer.

1. On dispute, the oracle sends a price request to `oovp.voting`, stamped with the assertion/dispute context so voters and tooling can identify it.
...
3. Votes are weighted by each voter's **staked OOVP**, snapshotted at commit time so stake cannot be added mid-round to swing a result.
