...
Pythia is governed by **PYTHIA** stakers. The same DVM that resolves oracle disputes also ratifies protocol changes — a governance proposal is submitted as an oracle **assertion**, and if it settles as _true_ (undisputed, or upheld by a DVM vote), it becomes executable on-chain.
...
|---|---|
| `pythiagovern` | Holds proposals, submits them as assertions, and executes approved ones. |
| `pythiaoorcle` | Receives the proposal assertion and settles its truth. |
| `pythiavoting` | Resolves the assertion if it is disputed. |
| `pythiastake1` | Weights the vote and slashes wrong/absent voters. |
| `pythiatoken1` | The PYTHIA token: bonds, stake, and rewards. |

## The PYTHIA token

PYTHIA is the protocol token. It is used for:
...
{% hint style="info" %}
The on-chain token symbol is **PYTHIA** (4 decimals).
{% endhint %}
...
2. The authorized proposer submits it through `pythiagovern::propose` and funds the bond.
